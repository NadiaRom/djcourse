...
## Довгий => широкий. Стовпчики в рядки
...
Ура, наша перша функція! У перетворенні даних з широкого формату на довгий у нас зміннються:
- назва колонки зі значеннями
- сам датафрейм
...
У програмування принцип називається "DRY": Don't Repeat Yourself.  
До речі, всі бібліотеки в R — це набори функцій. Ви просто завантажуєте функції, які за вас написали інші.

## З'єднання даних (Join).
Це операція поєднання колонок з різних таблиць за спільною колонкою-ключем, яка однакова в обох таблицях. Таких колонок може бути декілька. 
...
```{r}
df <- read.csv("https://github.com/NadiaRom/djcourse/raw/master/uku2020/day3/covid19_clean.csv") %>%
...
```{r}
ggplot(df, aes(x = day_since_1death, y = active_cases, group = country, color = country)) +
  geom_line() +
  facet_wrap(~country) +
  scale_y_log10(labels = comma) + 
  scale_color_manual(values = c("red", "grey", "grey", "grey",
                                "green", "blue", "grey", "grey",
                                "orange", "grey", "yellow", "grey")) +
  theme_minimal()
